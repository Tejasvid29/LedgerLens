 ...
 * wallet, so no client state or fetch code is needed here at all. AddWalletForm
 * and RemoveWalletButton are the only client islands, for form state and the
 * remove-confirm step respectively.
 */
      ...
      {wallets.length > 0 && (
        <div className="border border-rule bg-white">
          ...
          {wallets.map((w) => (
            <div
              key={w.id}
              className={`ledger-row flex items-start justify-between gap-2 px-4 py-3 text-sm hover:bg-indigo/5 ${
                selectedId === w.id ? 'bg-indigo/10' : ''
              }`}
            >
              <Link href={`/?wallet=${w.id}`} className="min-w-0 flex-1">
              ...
              <RemoveWalletButton walletId={w.id} isSelected={selectedId === w.id} />
            </div>
          ))}
        </div>
      )}
